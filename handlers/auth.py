...
async def _send_auth_success(websocket, user_id, username, user, is_new_user, connected_clients, server_data, client_ip, is_cracked=False):
    ...
    user_for_client = {k: v for k, v in user.items() if k != "validator" and k != "password_hash"}
    ...
    connected_usernames = server_data.get("connected_usernames", {}) if server_data else {}
    was_online = username in connected_usernames and connected_usernames.get(username, 0) > 0
...
async def handle_cracked_auth(websocket, data, config_data, connected_clients, client_ip, server_data=None):
    username = data.get("username", "").strip()
    password = data.get("password", "")
    
    if not username or not password:
        await send_to_client(websocket, {"cmd": "auth_error", "val": "Username and password required"})
        return False
    
    success, user_id, error = users.authenticate_cracked_user(username, password)
    if not success:
        await send_to_client(websocket, {"cmd": "auth_error", "val": error or "Authentication failed"})
    ...
    return await _send_auth_success(websocket, user_id, username, user, False, connected_clients, server_data, client_ip, is_cracked=True)


async def handle_cracked_register(websocket, data, config_data, connected_clients, client_ip, server_data=None):
    allow_registration = get_config_value("cracked", "allow_registration", default=True)
    if not allow_registration:
        await send_to_client(websocket, {"cmd": "auth_error", "val": "Registration is disabled"})
        return False
    ...
    default_roles = get_config_value("DB", "users", "default", "roles", default=["user"])
    success, user_id, error = users.register_cracked_user(username, password, default_roles)
    
    if not success:
        await send_to_client(websocket, {"cmd": "auth_error", "val": error or "Registration failed"})
        Logger.error(f"Client {client_ip} failed registration: {error}")
        return False
    
    user = users.get_user(user_id)
    Logger.add(f"New cracked user registered: {username} (ID: {user_id})")
    
    return await _send_auth_success(websocket, user_id, username, user, True, connected_clients, server_data, client_ip, is_cracked=True)
...
async def handle_authentication(websocket, data, config_data, connected_clients, client_ip, server_data=None, validator_key=None):
    ...
    return await _send_auth_success(websocket, user_id, username, user, is_new_user, connected_clients, server_data, client_ip)
